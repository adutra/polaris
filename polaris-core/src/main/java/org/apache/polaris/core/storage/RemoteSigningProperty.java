...
public enum RemoteSigningProperty {
  ...
  /**
   * The storage type targeted by the request to sign. Must be a valid {@link
   * PolarisStorageConfigurationInfo.StorageType} name. Required.
   */
  STORAGE_TYPE("polaris.remote-signing.storage.type"),

  /**
   * The HTTP(S) storage URI targeted by the request to sign. Optional. Included only for S3, to
   * help the signing endpoint determine whether path-style or virtual-hosted-style URLs are being
   * used.
   */
  STORAGE_ENDPOINT("polaris.remote-signing.storage.endpoint");
