...
RUN ./gradlew --no-daemon --info ${ECLIPSELINK_DEPS+"-PeclipseLinkDeps=$ECLIPSELINK_DEPS"} -PeclipseLink=$ECLIPSELINK clean :polaris-service:build
...
COPY --from=build --chown=polaris:polaris polaris-service/build/quarkus-app/lib/ /deployments/lib/
COPY --from=build --chown=polaris:polaris polaris-service/build/quarkus-app/*.jar /deployments/
COPY --from=build --chown=polaris:polaris polaris-service/build/quarkus-app/app/ /deployments/app/
COPY --from=build --chown=polaris:polaris polaris-service/build/quarkus-app/quarkus/ /deployments/quarkus/
