...
public class PolarisStorageIntegrationProviderImpl implements PolarisStorageIntegrationProvider {
  ...
  public PolarisStorageIntegrationProviderImpl(StorageConfiguration storageConfiguration) {
    this(stsClientSupplier(storageConfiguration), gcpCredentialsProvider(storageConfiguration));
  }

  private static Supplier<StsClient> stsClientSupplier(StorageConfiguration config) {
    return () -> {
      StsClientBuilder stsClientBuilder = StsClient.builder();
      if (config.awsAccessKey().isPresent() && config.awsSecretKey().isPresent()) {
        LOGGER.warn("Using hard-coded AWS credentials - this is not recommended for production");
        StaticCredentialsProvider awsCredentialsProvider =
            StaticCredentialsProvider.create(
                AwsBasicCredentials.create(
                    config.awsAccessKey().get(), config.awsSecretKey().get()));
  ...
  private static Supplier<GoogleCredentials> gcpCredentialsProvider(StorageConfiguration config) {
    return () -> {
      if (config.gcpAccessToken().isEmpty()) {
        try {
        ...
        AccessToken accessToken =
            new AccessToken(
                config.gcpAccessToken().get(),
                new Date(Instant.now().plus(config.gcpAccessTokenLifespan()).toEpochMilli()));
